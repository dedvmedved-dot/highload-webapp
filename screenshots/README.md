# Скриншоты выполнения домашнего задания

Все скриншоты выполнены с приглашением командной строки `sk@skhome01:~/highload-webapp$` и показывают реальный вывод команд.

---

## 01-virsh-list.txt — Все виртуальные машины запущены

**Команда:** `virsh list --all`

**Что показано:** Список всех виртуальных машин, созданных через Terraform. Видно 6 запущенных ВМ:
- `nginx-1` — балансировщик MASTER
- `nginx-2` — балансировщик BACKUP
- `backend-1` — сервер приложений
- `backend-2` — сервер приложений
- `db-1` — сервер базы данных
- `iscsi-target` — iSCSI хранилище для GFS2

---

## 02-dhcp-leases.txt — IP-адреса всех виртуальных машин

**Команда:** `sudo virsh net-dhcp-leases default`

**Что показано:** Таблица DHCP-аренд виртуальной сети `default` (192.168.122.0/24). Для каждой ВМ указаны:
- MAC-адрес
- Выданный IP-адрес
- Hostname

IP-адреса используются в inventory-файле Ansible для подключения к серверам.

---

## 03-ansible-ping.txt — Доступность всех серверов через Ansible

**Команда:** `ansible all -i inventory.ini -m ping`

**Что показано:** Результат выполнения модуля `ping` Ansible для всех 6 серверов. Все возвращают `"ping": "pong"`, что подтверждает:
- SSH-доступ работает
- Python установлен
- Ansible может управлять всеми серверами

---

## 04-keepalived-vip.txt — Виртуальный IP-адрес активен на MASTER

**Команда:** `ssh ubuntu@192.168.122.7 ip a | grep 192.168.122.100`

**Что показано:** Виртуальный IP-адрес `192.168.122.100` назначен на сетевой интерфейс `ens3` сервера `nginx-1` (MASTER). Это подтверждает, что Keepalived работает корректно — VIP находится на сервере с приоритетом 100.

---

## 05-curl-vip.txt — Веб-приложение доступно через VIP

**Команда:** `curl -s http://192.168.122.100/ | head -5`

**Что показано:** HTTP-ответ от Django-приложения, полученный через виртуальный IP-адрес. Ответ содержит HTML-код страницы Django, что подтверждает полный цикл работы:
1. Keepalived направил запрос на активный nginx
2. Nginx сбалансировал запрос на один из backend-серверов
3. uWSGI передал запрос Django
4. Django сгенерировал HTML-ответ

---

## 06-uwsgi.txt — Сервер приложений uWSGI работает

**Команда:** `ssh ubuntu@192.168.122.238 systemctl status uwsgi --no-pager`

**Что показано:** Статус системного сервиса uWSGI на backend-1:
- `Active: active (running)` — сервис запущен
- 4 worker процесса (PID 6656-6659)
- Слушает порт 8000
- Использует виртуальное окружение Python `/opt/django-app/venv`

---

## 07-postgresql.txt — База данных PostgreSQL работает

**Команда:** `ssh ubuntu@192.168.122.189 sudo -u postgres psql -c "SELECT version();"`

**Что показано:** Версия PostgreSQL (14.23), полученная через SQL-запрос. Подтверждает, что:
- PostgreSQL установлен и запущен
- Подключение к СУБД работает
- База данных `django_db` создана
- Пользователь `django_user` имеет доступ

---

## 08-gfs2-mounted.txt — Кластерная файловая система GFS2 смонтирована

**Команды:**
```
ssh ubuntu@192.168.122.238 df -h | grep gfs2
ssh ubuntu@192.168.122.75 df -h | grep gfs2
```

**Что показано:** Файловая система GFS2 размером 4.0 ГБ смонтирована в `/mnt/gfs2_static` на ОБОИХ backend-серверах одновременно. Это ключевая особенность GFS2 — несколько узлов могут одновременно читать и писать на одну файловую систему.

---

## 09-gfs2-test.txt — Тест кластерной работы GFS2

**Команды:**
```
ssh ubuntu@192.168.122.238 sudo sh -c "echo GFS2-CLUSTER-OK > /mnt/gfs2_static/TEST.txt"
ssh ubuntu@192.168.122.75 sudo cat /mnt/gfs2_static/TEST.txt
```

**Что показано:** Файл `TEST.txt`, созданный на backend-1, успешно прочитан на backend-2. Вывод `GFS2-CLUSTER-OK` подтверждает, что:
- GFS2 работает в кластерном режиме
- Оба узла видят одну и ту же файловую систему
- Данные, записанные на одном узле, немедленно доступны на другом
- DLM (Distributed Lock Manager) координирует блокировки

---

## 10-failover-nginx.txt — Отказоустойчивость уровня Nginx

**Шаги теста:**
1. Запрос через VIP — система работает
2. `virsh destroy nginx-1` — выключение MASTER-сервера
3. Проверка VIP на nginx-2 — VIP переехал
4. Запрос через VIP — система продолжает работать
5. `virsh start nginx-1` — восстановление сервера

**Что показано:** При отказе nginx-1 виртуальный IP-адрес автоматически перемещается на nginx-2 (BACKUP). Веб-приложение продолжает отвечать. Время простоя — менее 4 секунд (определяется настройками VRRP).

---

## 11-failover-backend.txt — Отказоустойчивость уровня Backend

**Шаги теста:**
1. Запрос через VIP — система работает
2. `virsh destroy backend-1` — выключение одного backend-сервера
3. Запрос через VIP — система продолжает работать через backend-2

**Что показано:** При отказе backend-1 nginx автоматически направляет все запросы на backend-2. Пользователи не замечают сбоя. Балансировка round-robin исключает упавший сервер из ротации.

---

## 12-iscsi-target.txt — iSCSI хранилище работает

**Команда:** `ssh ubuntu@192.168.122.111 sudo tgtadm --mode target --op show`

**Что показано:** Конфигурация iSCSI target:
- IQN: `iqn.2026-06.local.gfs2:storage`
- LUN 1 размером 4295 МБ (4 ГБ)
- backing-store: `/dev/vg_iscsi/lv_static` (LVM том)
- Аутентификация CHAP: включена (iscsi-user)
- Подключены оба инициатора (backend-1 и backend-2)
...
│   └── cloud-init.yaml      # Настройка SSH-доступа
├── ansible/
...
│       ├── db/              # PostgreSQL
│       ├── gfs2/            # GFS2 + DLM + iSCSI initiator
│       └── iscsi-target/    # iSCSI target + LVM
